<%@ Page Language="C#" MasterPageFile="~/Masters/ListMaster.master" AutoEventWireup="True"
    CodeBehind="UserSubList.aspx.cs"  Inherits="SKT.LeanMES.Web.User.UserSubList" Title="User List Page" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="GridviewContent" runat="Server">
    ...
    <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" EnablePaging="true" 
        ...
        TypeName="SKT.Common.Account.BLL.Users" SelectMethod="GetAllSub" SelectCountMethod="GetCount">
        <SelectParameters>
    ...
    <script type="text/javascript">
        ...
        function Add() {
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/User/UserEdit.aspx?name=Account_UserAdd&ID=-1&IsGroup=1";
        ...
        function Edit() {
            ...
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/User/UserEdit.aspx?name=Account_UserEdit&ID=" + idStr+"&IsGroup=1";
        ...
        function View() {
            var idStr = getOneRecordId();
            if (idStr == "") return false;
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/User/UserView.aspx?name=Account_UserView&ID=" + idStr + "&IsGroup=1";
        ...
        function ResetPwd() {
            var idStr = getOneRecordId();
            if (idStr == "") return false;
            dialog({ title: "初始化密码", src: "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/User/UserChangePwd.aspx?name=Account_ChangePwd&ID=" + idStr + "&IsGroup=1", width: 600, height: 400 });
            return false;
        ...
        function UnlockUser() {
            ...
            if (lock.indexOf("正常") == -1) {
                if (confirm("是否确定解锁当前选中的用户？")) {
                    var ajax = SKT.LeanMES.Web.AjaxServices.AjaxAccount.UnlockUser(idStr,"1");
                    if (ajax.error != null) {
                        alert(ajax.error.Message);
                        return false;
                    }
        ...
        function LogoffUser() {
            ...
            if (confirm("注销用户功能将会使用系统前台登录的用户退出系统，是否确定注销选中的用户？")) {
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxAccount.LogoffUser(idStr,"1");
                if (ajax.error != null) {
                    alert(ajax.error.Message);
                    return false;
                }
        ...
        function AssignUserToRole() {
            var idStr = getOneRecordId();
            if (idStr == "") return false;
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/Role/RolesInUser.aspx?name=Account_RolesInUser&ID=" + idStr + "&IsGroup=1";
            dialog({ title: "分配角色", src: openWinUrl, width: 815, height: 490 });
        }

        function Import() {
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/User/UserImport.aspx?name=Account_UserImport&IsGroup=1";
        ...
        function AssignPinter() {
            ...
            if (idStr == "")
                return;
            var IsGroup = "1";
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxAccount.GetGroupName(IsGroup);
            if (ajax.error != null) {
                alert(ajax.error.Message);
                return;
            }
