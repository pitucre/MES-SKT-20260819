<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ImportHelp.aspx.cs" Inherits="SKT.MES.Web.Help.ImportHelp" %>
...
<body style="margin: 0px; padding: 0px; font-family: Verdana, 微软雅黑,黑体, 宋体; background: #faf9f9;">
    ...
    <div style=" height:23px; background:#eeeeee; padding:7px 3px 3px 20px;">
        <div style="line-height:19px; margin-left:5px; border:1px solid #faf9f9; width:75px; text-align:center; background:#faf9f9; float:left;"><a href="CreateMenus.aspx" target="_self">生成菜单</a></div>
        <div style="line-height:19px; margin-left:5px; border:1px solid #faf9f9; width:75px; text-align:center; background:#f1f9f9; float:left;"><a href="#">导入帮助</a></div>
    </div>
            ...
            <div style="position: relative; height:80px;">
                ...
                <div style="position: absolute; left: 610px; top: 20px;">
                    <asp:Button ID="btnUpload" runat="server" Text="上传文件" 
                        onclick="btnUpload_Click" CssClass="button" Height="46px"/>
                </div>
            </div>
            <div >
    ...
    <script language="javascript" type="text/javascript">
        $(function () {
            $("#<%=this.btnUpload.ClientID %>").click(function () {
                ...
                if (fileName == "") {
                    $("#<%=this.lblMessage.ClientID %>").text("请选择要上传的Excel文件！");
                    return false;
                }
                var fileExt = fileName.substring(fileName.lastIndexOf(".")+1);
                 
                if (fileExt.toLowerCase() != "xls" && fileExt.toLowerCase() != "xlsx") {
                    $("#<%=this.lblMessage.ClientID %>").text("文件格式不正确，只能上传Excel文件！");
                    return false;
                }
