<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Masters/ListMaster.master"
    CodeBehind="PackGRNSuplyList.aspx.cs" Inherits="SKT.LeanMES.Web.SuplyMaterial.PackGRNSuplyList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="SearchContent" runat="server" ViewStateMode="Enabled">
    ...
    <table class="EditeContentTable" width="100%">
        ...
        <tr>
            ...
            <td class="Field2">
                 <input type="text" id="txtPurOrderNo" class="TextBox" runat="server" />
            </td>
        </tr>
    </table>
    <!--打印状态的信息提示区域-->
...
<asp:Content ID="Content2" ContentPlaceHolderID="GridviewContent" runat="server">
    <asp:GridView ID="GridView1" AutoGenerateColumns="false" runat="server"  OnRowDataBound="GridView1_OnRowDataBound" >
        ...
        <%--    <asp:BoundField DataField="Quantity" HeaderText="<%$Resources:lang,MinPackageQty %>"  HeaderStyle-Width="120px"/>--%>
    ...
    <script type="text/javascript">
        ...
        function UnBindPacking() {
            ...
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxMaterial.UnPack(cartonSn,1);
            if (ajax.error != null) {
        ...
        function RepairPrint() {
            ...
            setTimeout(function () {
                try {
                    ...
                    mesLabLabelPrint();
                    if (snStr != "")
                        creatservicelog("补打包装箱", "供应商物料管理|供应商包装箱列表", "重打印", snStr, "供应商重打印包装箱【" + snStr + "】");
                }
                catch (e) {
                    $("#lblMessage").html(e);
                    creatservicelog("补打包装箱", "供应商物料管理|供应商包装箱列表", "重打印", "打印异常", "供应商重打印包装箱异常：【" + e + "】");
                }
            }, 30);
        }
        ...
        function chooseVendor() {
            ...
            if (UserType == "-1") {
                ...
                var VendorCode = "";
                VendorCode = GetVenCode(UserId);
                searchCondition = " VendorCode='" + VendorCode + "'";
            }
             dialog({ title: "<%=Resources.Common.ChooseWindow %>",
                 src: "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/Framework/ChoosePage.aspx?PageId=34&PageCondition=" + searchCondition + "&CallBackFunc=setVendor&Multiple=false&rnd=" + Math.random(), width: 650, height: 350
            });
        }
        //取供应商编码
        function GetVenCode(userid) {
            var code = "";
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxMaterial.GetVenCodeByUserId(userid);
            if (ajax.error == null) {
                var entity = ajax.value;
                code = entity.VendorCode;
            }
            else {
                //alert(ajax.error.Message);
                code = "";
        ...
        //var labelType = 2;          //标签类型  (1.GRN 2.包装)
        //var labelSequence = 1;      //标签序号        var labelType = 2;          //标签类型  (1.GRN 2.包装)
        var labelType = -14;
        var labelSequence = 8;      
