<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/EditMaster.master" AutoEventWireup="true"
    CodeBehind="EquipmentInspectionTemplateEdit.aspx.cs" Inherits="SKT.LeanMES.Web.Equipment.EquipmentInspectionTemplateEdit" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="EditContent" runat="server">
    ...
    <table id="tblExpand" cellspacing="0" cellpadding="4" style="        border-width: 0px;
        width: 100%;
        ...
        <tr class="ListTableHeader" style="        text-align: center">
            ...
            <th scope="col" onclick="chooseInspectionItem(null);" style="        color: #0066CC;
        cursor: pointer;
        width: 5%;"><span>+添加点检项目</span>
            </th>
                ...
                if (xhr.readyState == 4) {
                    ...
                    if (200 == xhr.status) {
                        ...
                        if (entity.code == 1) {
                            ...
                            $("#div-upload").parent().parent().parent().find("td:last").append("<img  class=\"layui-upload-img\" src=\"<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/ESOP/DownLoad.aspx?Action=EquipmentTemplateItemFile&amp;fileName=" + entity.data.FileName + "\">");
                            //$("#IsUpLoadImg").val(1);  
                            return;
    ...
    <script language="javascript" type="text/javascript">
        ...
        $(function () {
            ...
            $("#<%=this.ddlInspectionType.ClientID %>").change(function () {
                ...
                // var entity = SKT.LeanMES.Web.Equipment.EquipmentInspectionTemplateEdit.GetInspectionTypeInf(inspecType).value;
        ...
        function addDetail(entity, i) {
            ...
            cell.align = "center";
            cell.className = "Field";
            //alert(typeof $(tab.rows[i].cells[12]).attr("test","1233"))
            if (entity.OpenName != null && entity.OpenName != "") {
                cell.innerHTML = "<img class=\"layui-upload-img\" src=\"<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/ESOP/DownLoad.aspx?Action=EquipmentTemplateItemFile&amp;fileName=" + entity.OpenName + "   \" />";
               // alert($(cell).html());
               // cell.setAttribute("filenameserver", entity.OpenName);
                //  cell.setAttribute("filenameserver", entity.OpenName);
        ...
        function selectInspectionmethods(obj) {
            ...
            dialog({
                title: "<%=Resources.Common.ChooseWindow %>"
                , src: "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/Framework/ChoosePage.aspx?PageId=822&CallBackFunc=getChooseValuesselectInspectionmethods&PageCondition= " + searchCondition + "&Multiple=true&rnd=" + Math.random(), width: 650, height: 350
            });
        }

        function getChooseValuesselectInspectionmethods(list) {
            var Inpsectionmethods = [];
            var myValue = $(rowObj2).find(".txtInspectionmethods").val();
            for (var i = 0; i < list.length; i++) {
                Inpsectionmethods.push(list[i][1]);
            }
            if (myValue != "") {
                $(rowObj2).find(".txtInspectionmethods").val(myValue + "," + Inpsectionmethods.join(','));
            }
            else {
                $(rowObj2).find(".txtInspectionmethods").val(Inpsectionmethods.join(','));
            }
        ...
        function Save() {
            ...
            var entity = {};
            entity.InspectionTemplateId = $("#<%=this.txtHideInspectionTemplateId.ClientID %>").val();
