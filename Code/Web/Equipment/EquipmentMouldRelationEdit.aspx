<%@ Page Language="C#" MasterPageFile="~/Masters/EditMaster.master" AutoEventWireup="True"
    CodeBehind="EquipmentMouldRelationEdit.aspx.cs" Inherits="SKT.LeanMES.Web.Equipment.EquipmentMouldRelationEdit" Title="Edit EquipmentMouldRelation" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="EditContent" runat="Server">
    ...
    <table width="100%" class="EditeContentTable">
        <tr>
            ...
            <td class="Field1" >
                ...
                <asp:HiddenField ID="HiddenEquimentId" runat="server" Value="-1" />
            </td>
        ...
        <table class="EditeContentTable" width="100%">
            <tr>
                <td class="Label" style="width: 49%; text-align: center; font-weight: bold;">
                    <asp:Label ID="lbl1" runat="server" Text="<%$ Resources:lang, ListPtionalAssociatedMolds %>"></asp:Label>
                </td>
                <td class="Label" style="width: 2%; text-align: center;"></td>
                <td class="Label" style="width: 49%; text-align: center; font-weight: bold;">
                    <asp:Label ID="lbl2" runat="server" Text="<%$ Resources:lang, ListPtionalAssociatedMolds %>"></asp:Label>
                </td>
            </tr>
            <tr>
                <td class="Field" style="width: 45%; vertical-align: top;">
                    <iframe name="frmRoleChooseList" id="fromChooseList1" frameborder="0" style="width: 99%; height: 265px;"
                        src="MouldPreEquipment.aspx?EquimentId=<%= Request.QueryString["ID"] %>"></iframe>
                </td>
    ...
    <script type="text/javascript">
        ...
        function getChooseValue(list) {
            if (temp == 1) {
                ...
                $("#<%=this.HiddenEquimentId.ClientID%>").val(list[0][0]);
                var iframe2 = document.getElementById("IfrHasChooseList");
                iframe2.src = "MouldInEquipment.aspx?EquimentId=" + list[0][0] + "";
      
                var iframe1 = document.getElementById("fromChooseList1");
                iframe1.src = "MouldPreEquipment.aspx?EquimentId=" + list[0][0] + "";
            }
            <%--  if (temp == 2) {
    ...
    <script type="text/javascript">
        var elementById = -1;
        $(function() {
            elementById = $("#<%=HiddenEquimentId.ClientID%>").val();
            if (elementById != -1) {

                var iframe2 = document.getElementById("fromChooseList1");
                iframe2.src = "MouldPreEquipment.aspx?EquimentId=" + elementById + "";

                var iframe2 = document.getElementById("IfrHasChooseList");
                iframe2.src = "MouldInEquipment.aspx?EquimentId=" + elementById + "";
            }
        });

        function btnChooseOnClick(index) {
            elementById = $("#<%=this.HiddenEquimentId.ClientID%>").val();
            if (elementById == -1)
            {
                alert("请选择模具编码！");
                return;
            ...
            if (index == 0) {
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxEquipmentMouldRelation.SaveMouldInEquiment(elementById, itemIdString);
                if (ajax.error != null) {
                    alert(ajax.error.Message);
                    return;
                }
                $("#<%=this.HiddenEquimentId.ClientID%>").val(ajax.value);                
            }
            else {
                /*从机种设备关系表(Basal_EquipmentItemRelation)中删除数据*/
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxEquipmentMouldRelation.RemoveMouldOutEquipment(elementById, itemIdString);
                if (ajax.error != null) {
                    alert(ajax.error.Message);
                    return;
                }
            ...
            if(equipmentItemRelationId==-1){                    
                var iframe1 = document.getElementById("fromChooseList1");
                iframe1.src = "MouldPreEquipment.aspx?EquimentId=" + elementById + "";
