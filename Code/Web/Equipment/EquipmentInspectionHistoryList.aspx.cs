...
{
    public partial class EquipmentInspectionHistoryList : BasePage
    {
            ...
            if (this.IsPostBack)
            {
                try
                {
                    ...
                    if (this.hdnOperate.Value.ToLower() == "exportexcel")
                    {
                        ...
                        var list = new SKT.LeanMES.Equipment.BLL.EquipmentInspectionTemplateItem().GetAllEquipmentInspectionHistoryList(0, int.MaxValue, sort, searchSettings);
                        NPOIHelper.Export(list, this.GridView1, "设备点检操作记录-" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx");
                    }
                }
                catch (Exception ex)
                {
