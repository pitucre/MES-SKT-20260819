...
{
    public partial class MaterialSysConfigList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (configTypeId != -1)
            {
                searchSettings.ExtensionCondition = " ConfigTypeId = " + this.ddlConfigType.SelectedValue + "";
            }
            if (configTypeId == -1)
            {
                searchSettings.ExtensionCondition += " ConfigCode <>'fixed'";
            }
            else
            {
                searchSettings.ExtensionCondition += " and  ConfigCode <>'fixed'";
            }
            ...
            if (IsPostBack)
            {
                if (Request.Form["hdnOperate"].ToLower() == "delete")
                {
                    try
                    {
                        ...
                        bll.Delete(Request.Form["hdnIdString"].ToString(), AccountController.GetCurrentUser().UserName, 0);
