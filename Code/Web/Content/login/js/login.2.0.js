...
$(function () {
    ...
    $("#qrcode").qrcode({
        ...
        width: 111,
        height: 111,
        text: window.location.origin + _webRoot + "/MobileApp/Login.aspx",
        typeNumber: -1,/*计算模式*/
    ...
    $("#qrcode").attr("link", window.location.origin + _webRoot + "/MobileApp/Login.aspx");
...
function InitLang() {
    if (_LANGUAGE === "zh-cn") {
        ...
        $("#version_name").text("(Professional)")
    }
}
...
function GetZhuSuCookie() {
    //如果机台Cookie为空，则弹窗机台页面
    if ($.cookie("jt") == "") {
    ...
    if ($.cookie("gd") == "") {
        /*dialog({ title: "选择工单机台", src: webroot + "/Client/SwitchOrderMachine.aspx?name=Switch_OrderAndMachine&EquimetCode=" + ($.cookie("jt") + "&rnd=" + Math.random(), width: 750, height: 600 });*/
        return;
    }
}
...
jQuery(function () {
    ...
    if ($.cookie("zhusu") == "true") {
        $("#zhusuInput").attr("checked", true);
    }
    //if ($.cookie("rmb") == "true") {
    //    $("#rmbme").attr("checked", true);
    //    $("#userName").val($.cookie("account"));
...
function doLogin() {
    ...
    var zhusu = ($("#zhusuInput")[0].checked) ? 1 : 0;
    var _lang = $("#multiple_language").attr("data");
    $.ajax({
        type: 'post',
        url: _WEB_ROOT + "Handler/MobileAppLogin.ashx?Action=Login&rnd=" + Math.random(),
        async: true,
        data: { UserName: username, Pwd1: JsDesEncrypt(passwd), lang: _lang, rmb: rmb, multiplant: _multiplant, site: _site },
        beforeSend: function () {
            $("#msg").html(loginmsg);
            if (!isMultiplant)
                $("#loginbtn").html("正在登录...");
        ...
        success: function (data) {
            ...
            if (data == "") {
                if ($("#zhusuInput").is(':checked')) {
                    location.href = _WEB_ROOT + "Client/MachinedInjectionMolding.aspx"
                }
                else {
                    location.href = _WEB_ROOT + "Framework/Console.aspx?isLogin=1";
                }
            }
            else {
...
function getDBLinks() {
    $.ajax({
        type: 'post',
        url: _WEB_ROOT + "Handler/Multiplant.ashx?action=GetMultiDbLinks&rnd=" + Math.random(),
        async: true,
        success: function (data, status) {
            if (status == "success") {
                if (data != "") {
                    ...
                    setMultiplantInfo(jsonData.dblinks);
                }
                else {
                    isMultiplant = false;
                }
...
function setMultiplantInfo(siteList) {
    $("#version_name").html($("#version_name").html().replace(")", " - 多工厂)"));
    $("#loginbtn").html("<div style='text-align:center;'><span id='btn_text'>选择工厂登录</span><span class='multiplant-drp-arrow'></span></div>");
    var multiplantList = "";
    multiplantList += "<div class='multiplant-drp-top'></div>";
    for (var i = 0; i < siteList.length; i++) {
        multiplantList += "<div class='multiplant-drp-item' data='" + siteList[i].id.toString() + "'>" + siteList[i].site + "</div>";
    }
    multiplantList += "<div class='multiplant-drp-btm'></div>";
    $("#login_button").append("<div class='multiplant-drp'>" + multiplantList + "</div>");

    $(".multiplant-drp-item").hover(function () {
        $(this).addClass("multiplant-drp-item-active");
    },
        function () {
            $(this).removeClass("multiplant-drp-item-active");
        });
    $("#login_button").hover(
        function () { },
        function () {
            $(".multiplant-drp").slideUp();
        });
    $(".multiplant-drp-item").click(function () {
        $("#btn_text").html($(this).html());
        $("#btn_text").attr("data", $(this).attr("data"));
        $(".multiplant-drp").slideUp();
        doLogin();
    });

    $("#btn_text").html($.cookie("multiplant"));
    $("#btn_text").attr("data", $.cookie("multiplantid"))
}
