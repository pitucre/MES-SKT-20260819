    ...
    showTree: function (opt) {
        ...
        if (opt.alert == 'alert') {
            $(this).click(function () {
                ...
                $.each(dataList, function (i, n) {
                    if (n.ParentId == -1) {
                        html += '<li class="expandable"><div class="hitarea"></div><span class="folder" val="' + n.OrganizationId + '" loaded="false" departno="'+n.DepartNo+'">' + n.DepartName + '</span></li>';
                    }
                });
                html += '</ul></div>';
                html = $(html);
            ...
            $.each(dataList, function (i, n) {
                if (n.ParentId == -1) {
                    $(_self).append('<li class="expandable"><div class="hitarea"></div><input type="checkbox" value="' + n.OrganizationId + '" parentId="' + n.ParentId + '" name="chbId" class="checkbox" onclick="chkItem(this)"/><span class="folder" val="' + n.OrganizationId + '" loaded="false"  departno="' + n.DepartNo + '" onclick="chkItem1(this)">' + n.DepartName + '</span></li>');
                }
            });
            initTree($(_self));
        }
        ...
        function initTree(t) {
            t.find('span[loaded=false]').each(function () {
                ...
                $(this).prev().prev().bind('click', function () {
        ...
        function addChindList(evt) {
            ...
            $.each(dataList, function (i, n) {
                if (n.ParentId == id) {
                    str += '<li class="expandable"><div class="hitarea"></div><input type="checkbox" value="' + n.OrganizationId + '" parentId="' + n.ParentId + '" name="chbId" class="checkbox" onclick="chkItem(this)"/><span class="folder" val="' + n.OrganizationId + '" loaded="false"  departno="' + n.DepartNo + '"  onclick="chkItem1(this)">' + n.DepartName + '</span></li>';
                }
            });
            if (str) {
...
function chkItem(obj) {
    if ($(obj)[0].checked) {
        ...
        GetDepartmentMemeber($(obj).val(), $(obj).next().attr("DepartNo"));
    }
    else {
        $(obj)[0].checked = false;
        GetDepartmentMemeber(-1, $(obj).next().attr("DepartNo"));
    }
}

function chkItem1(obj) {
    var cbk = $(obj).parent().children("input[type=checkbox]");
    $(cbk[0])[0].checked = !$(cbk[0])[0].checked;
    if ($(cbk[0])[0].checked) {
        $(".treeview input[type='checkbox']").each(function () {
            $(this)[0].checked = false;
        });
        $(cbk[0])[0].checked = true;
        GetDepartmentMemeber($(cbk[0]).val(), $(cbk[0]).next().attr("DepartNo"));
    }
    else {
        $(cbk[0])[0].checked = false;
        GetDepartmentMemeber(-1, $(cbk[0]).next().attr("DepartNo"));
    }
}