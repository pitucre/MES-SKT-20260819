    ...
    $.fn.gantt = function (options) {
        ...
        var core = {
            ...
            render: function (element) {
				...
				$dataPanel.css({ height: $(".contain").height() });
            ...
            leftPanel: function (element) {
                ...
                debugger;
                var heights = tools.getCellSize() * element.headerRows;

                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    .append($('<div class="row spacer"><div class="out" style="border-top: ' + heights + 'px #D6D3D6 solid;position: relative;border-left: 230px #BDBABD solid;height: 0px;width: 0px;"> <b style="display: block;position: absolute;top: -' + (heights - 10) + 'px;left: -90px;">时间</b> <b style="display: block;position: absolute;top: -' + (heights - 40 < 21 ? 21 : 32) + 'px;left: -220px;">工单号(工单进度)</b> </div></div>')
                ...
                $.each(element.data, function (i, entry) {
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        if (entry.desc) {
                            ...
                            entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '"><div class="Bars"><div style="width:' + entry.desc + ';"> <span>' + entry.desc + '</span></div></div></span>');
                            entries.push('</div>');
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                default:
                                    ...
                                    _bar.css({ 'margin-top': top, 'margin-left': Math.floor(cFrom-24) });
        ...
        var tools = {
            ...
            parseDateRange: function (from, to) {
				...
				var pages = $(window).width() / 30;
                var ret = [];
                var i = 0;
                do {
                    ret[i++] = new Date(current.getTime());
                    current.setDate(current.getDate() + 1);
                } while (current.getTime() <= to.getTime() ||i<pages);
                return ret;
