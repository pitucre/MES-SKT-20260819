<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ViewMaster.master" AutoEventWireup="true"
    CodeBehind="MaterialScrap.aspx.cs" Inherits="SKT.LeanMES.Web.Material.MaterialScrap" %>

<asp:Content ID="Content1" ContentPlaceHolderID="viewcontent" runat="server">
    <table width="100%" class="EditeContentTable">
<%--        <tr class="Label">
            <td align="left">
                <span class="information16"></span>请输入或扫描要报废的物料条码和库位条码
            </td>
            <td align="right">
        ...
        <tr>
            ...
            <td class="Field1">
                <input type="text" id="txtQty" class="TextBox" value="0" disabled="disabled" onafterpaste="if(isNaN(value))execCommand('undo')"
                    style="width: 100px; height: 25px; text-transform: uppercase; font-size: 15px;
                    font-weight: bold; text-align: right" isnumber="1" isrequired='1' />
    ...
    <div id="tblInfo" style="text-align: left; color: #810000; height: 25px; line-height: 22px;
        ...
        <img src="../Content/images/icon/comment.png" style="vertical-align: middle;" alt="" />您可以扫描物料条码和库位条码来报废。
    </div>
    ...
    <script type="text/javascript">
        ...
        function GetQuantityByGRN(grn, barCode) {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxMaterial.GetQuantityByQty(grn, barCode, 2);
            if (ajax.error != null) {
        ...
        function Save() {
            ...
            setTimeout(function () {
                ...
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxMaterial.ScrapMaterial(grn, warCode);
                if (ajax.error != null) {
                    $("#txtGRN").focus();
                    $("#txtGRN").select();
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    r += "<td>" + list[i].SerialNumber + "</td><td>" + list[i].ItemName + "</td><td>" + list[i].CBarCode + "</td><td>" + list[i].BalanceQty + "</td><td>" + list[i].ModifyBy + "</td><td>" + list[i].PackTime + "</td>";
                    r += "</tr>";
                }
                ...
                $("#tblInfo").html("<img src=\"../Content/images/icon/comment.png\" style=\"vertical-align:middle;\" alt=\"\"/>当前共有入库记录：<b>" + ($("#tblRecHistory tr").length - 1).toString() + "</b> 条,本次扫描新增加记录：<b>" + (list.length).toString() + "</b> 条");
        ...
        function clearScreen() {
            if (confirm("是否确定要清空退料列表？")) {
                ...
                var leftStr = "<tr id='trNewInfo' class='ListTableOddRow'><td colspan='7' style='text-align:center;'>暂无数据</td></tr>";
