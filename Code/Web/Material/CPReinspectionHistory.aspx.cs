...
{
    public partial class CPReinspectionHistory : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                strWhere = "ReinspectionNo like '%" + txtCheckNo.Text + "%'";
            }
            ...
            {
                strWhere = "CheckResult like " + dllStatus.SelectedValue + "";
            }
            if (!string.IsNullOrEmpty(txtCheckUserName.Text))
            {
                strWhere = "UserName like '%" + txtCheckUserName.Text + "%'";
            }
            if (!string.IsNullOrEmpty(txtSerialNumber.Text))
            {
                strWhere = "SerialNumber like '%" + txtSerialNumber.Text + "%'";
            }
        ...
        private DataTable GetDgvToTable(SearchSettings searchSettings)
        {
            ...
            dt = ListToDataTable(bll.GetCPReinspections(0, 100000, "", searchSettings));
            ...
            for (int i = 0, j = dt.Rows.Count; i < j; i++)
            {
                ...
                dr[3] = dt.Rows[i]["ItemName"].ToString();
                dr[4] = dt.Rows[i]["Quantity"].ToString();
                dr[5] = dt.Rows[i]["CWhCode"].ToString();
                dr[6] = dt.Rows[i]["CBarCode"].ToString();
                dr[7] = dt.Rows[i]["CheckResultName"].ToString();
                dr[8] = dt.Rows[i]["UserName"].ToString();
                dr[9] = dt.Rows[i]["CreateDateTime"].ToString();
                dr[10] = dt.Rows[i]["CheckNumber"].ToString();                
                dr[11] = dt.Rows[i]["Remark"].ToString();
