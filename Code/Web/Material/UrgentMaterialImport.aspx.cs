...
{
    public partial class UrgentMaterialImport : BasePage
    {
        public string ShortageXml = string.Empty;
        ...
        protected void Upload_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    DataTable userListTb = NPOIHelpers.Import(filePath);//SKT.LeanMES.Web.AppCode.Utility.ExcelHelper.HSSExcelToDataTable(filePath);
                    System.Text.StringBuilder strBuilder = new System.Text.StringBuilder();
                    if (userListTb.Columns[0].ColumnName != "物料编码" || userListTb.Columns[1].ColumnName != "急料截止日期")
                    {
                        WebHelper.ShowMessage("Excel文件与模板格式不匹配，请按照模板格式导入！");
                        return;
                    }
                    if (userListTb != null && userListTb.Rows.Count > 0)
                    {
                        ...
                        GridView1.DataBind();
                        ShortageXml = strBuilder.ToString();
                    }
