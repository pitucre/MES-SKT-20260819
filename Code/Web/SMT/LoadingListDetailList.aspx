<%@ Page Language="C#" MasterPageFile="~/Masters/EditMaster.master" AutoEventWireup="true"
    Inherits="SKT.LeanMES.Web.SMT.LoadingListDetailList" Title="MODEL List Page"
    CodeBehind="LoadingListDetailList.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="EditContent" runat="Server">
    ...
    <div class="ListTableTitle" style="border-bottom: 0px; font-size: 12px; font-family: '宋体'">
        <%=Resources.Pages.LoadingList%>
    </div>
    <table class="ListTable" width="100%" cellpadding="3" cellspacing="0" border="0"
        id="tblMachineList">
        <tr class="ListTableHeader">
            <th width="3%">
                <input type="checkbox" name="chkAll" onclick="checkAll(this.checked);" />
            </th>
    ...
    <script type="text/javascript">
        ...
        /*增加 */
        function Add() {
            dialog({ title: "<%= Resources.lang.MachineAdd %>", src: "MachineEdit.aspx?name=MachineAdd&ID=-1", width: 700, height: 350, resizeable: true });
        }
        ...
        function GetMachineList() {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxServicesLoadingList.GetLoadinglist("-1");
            if (ajax.error != null) {
                alert(ajax.error.Message);
                return false;
            }
        ...
        /*根据机器ID获取Table列表*/
        function GetMachineTableList(LLID) {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxServicesLoadingList.GetLLMachine(LLID.toString());
            if (ajax.error != null) {
                alert(ajax.error.Message);
                return false;
            }
        ...
        /*根据Table ID 获取Slot列表*/
        function GetMachineTableSlotList(LLMID) {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxServicesLoadingList.GetLLDetail(LLMID.toString());
            if (ajax.error != null) {
                alert(ajax.error.Message);
                return false;
            }
        ...
        $(function () {
            ...
            for (var i = 0; i < list1.length; i++) {
                ...
                machineHtml += "<td><span class='itemCollapsed' onclick='toggleTable(this," + list1[i].ID + ")'></span>" + list1[i].SetupName + "</td>";
                machineHtml += "<td>" + list1[i].ItemStr + "</td>";
                machineHtml += "<td>" + list1[i].CustomerName + "</td>";
                machineHtml += "<td>" + list1[i].Revision + "</td>";
                machineHtml += "<td>" + list1[i].StatusStr + "</td>";
                machineHtml += "</tr>";

                /*2.绑定Table*/
                var list2 = GetMachineTableList(list1[i].ID);
                machineHtml += "<tr id='machine" + list1[i].ID.toString() + "' style='display:none; background:#faf9f9;' class='ListTableOddRow'><td colspan='6' style='padding-left:30px;'>";
                ...
                tableHtml += "<div style=' position:absolute; left:10px; top:7px;'><img src='<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/Content/Images/icon/list.png' alt='' style=' vertical-align:middle;'/>&nbsp;<%=Resources.lang.MachineTableList %></div>";
                tableHtml += "</div>";
                tableHtml += "</td></tr>";
                ...
                for (var j = 0; j < list2.length; j++) {
                    ...
                    tableHtml2 += "<td><span class='itemCollapsed' onclick='toggleSlot(this," + list2[j].ID + ")'></span>" + list2[j].MachineNo + "</td>";

                    tableHtml2 += "<td>" + list2[j].StatusStr + "</td>";
                    tableHtml2 += "</tr>";

                    /*3.绑定Slot*/
                    var list3 = GetMachineTableSlotList(list2[j].ID);
                    tableHtml2 += "<tr id='mtable" + list2[j].ID.toString() + "' style='display:none;' class='ListTableOddRow'><td colspan='6'>";
                    ...
                    slotHtml += "<div style=' position:absolute; left:10px; top:7px;'><img src='<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/Content/Images/icon/list.png' alt='' style=' vertical-align:middle;'/>&nbsp;<%=Resources.lang.TableSlotList %></div>";
                    slotHtml += "</div>";
                    slotHtml += "</td></tr>";

                    slotHtml += "<tr class='ListTableHeader'>";
                    slotHtml += "<th width='35%' align='left'><%= Resources.lang.SerialNumber%></th>";
                    slotHtml += "<th width='50%' align='left'><%= Resources.lang.ItemsName%></th>";
                    slotHtml += "<th align='left'><%= Resources.lang.MaterialCode%></th>";
                    slotHtml += "<th align='left'><%= Resources.lang.Frequency%></th>";
                    slotHtml += "<th align='left'><%= Resources.lang.Location%></th>";
                    ...
                    for (var k = 0; k < list3.length; k++) {
                        ...
                        slotHtml2 += "<td>" + list3[k].MachineTableSlotNo + "</td>";
                        slotHtml2 += "<td>" + list3[k].ItemName + "</td>";
                        slotHtml2 += "<td>" + list3[k].ItemCode + "</td>";
                        slotHtml2 += "<td>" + list3[k].BottomFrequency + "</td>";
                        slotHtml2 += "<td>" + list3[k].BottomLocation + "</td>";
                        slotHtml2 += "<td>" + list3[k].StatusStr + "</td>";
                        //                        slotHtml2 += "<td><select onchange='ChangeStatus(3,this)'>" + StatusSelect(list3[k].StatusID) + "</select><input type='hidden' value='" + list3[k].ID.toString() + "'/></td>";
                        slotHtml2 += "</tr>";
                    }
                    slotHtml = slotHtml + slotHtml2 + "</table>";
