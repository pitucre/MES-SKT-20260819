<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ProductionCollection.Master" AutoEventWireup="true" CodeBehind="EAIInspection.aspx.cs" Inherits="SKT.LeanMES.Web.Client.EAIInspection" %>
    ...
    <div class="client-center">
        ...
        <table id="tabTmplContent" class="EditeContentTable" style="width: 100%; margin-bottom: 5px;">
            ...
            <tr>
                ...
                <td class="Field2">
                    <select id="sltEAICode" isrequired="1">
            ...
            <tr>
                <td class="Field1" colspan="4" style="text-align: center;">
                    ...
                    <input id="btnSaveEAICode" onclick="saveInspectionOrder(this)" type="button"
    ...
    <script language="javascript" type="text/javascript">
        ...
        $(function () {
            ...
            setTimeout(
                    function () {
                        //加载按钮
                        loadClientButton('EAIInspection');
                    }, 10);
        ...
        $("#sltEAICode").change(function () {
            if ($(this).find("option:selected").attr("IOrderId") == "-1") {
                ...
                $("#tdVersion").text("");
                $("#txtsampleQty").val("");
                $("#btnCollectSN,#btnSaveEAICode").show();
                $(tab).find(".ListTableOddRow").empty().remove();
                return;
            }
            if ($(this).find("option:selected").attr("Status") != "-1") {
                $("#btnCollectSN,#btnSavEAICode").hide();
            }

            getEAIInspectionInfo(this.value);
            BindTab();
            getInspectionSN();
        ...
        function saveInspectionOrder(obj) {
            ...
            parent.EAISNArr.forEach(function (obj) {
                EAISNStr += obj.IOMemberId + "$" + obj.SN + "$" + obj.Result + "$" + obj.NCCode + "$" + obj.ScanTime + "^";
            });

            EAISNStr = EAISNStr.substring(0, EAISNStr.length - 1);
            ...
            entity.EAISNStr = EAISNStr;

            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxQC.CollectEAIInspecitonInfo(entity);
            if (ajax.error != null) {
                alert(ajax.error.Message);
            ...
            IOrderId = ajax.value;
            $("#sltEAICode").find("option:selected").attr("IOrderId", IOrderId);
        ...
        function collectSN() {
            ...
            dialog({
                title: "序列号采集",
                src: "<%= WebHelper.WebRoot %>/Client/EAIInspectionSN.aspx?IOrderId=" + IOrderId + "&rnd=" + Math.random(),
                width: 700,
                height: 450
            });
        }
        ...
        *生成末件单号
        **/
        function generateEAICode() {
            if (OrderId == -1) {
                return;
            }
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxQC.GenerateEAICode(OrderId, stationId, resourceId, LineId);
            if (ajax.error != null) {
                alert(ajax.error.Message);
                //写入日志
                SaveUserUILog("一般", stationId, resourceId, "", ajax.error.Message);
        ...
        function bindEAICode(list) {
            var html = "";
            var obj = $("#sltEAICode");

            for (var i = 0; i < list.length; i++) {
                html += "<option IOrderId='" + list[i].IOrderId + "' Status='" + list[i].Status + "' value='" + list[i].EAICode + "'>" + list[i].EAICode + "</option>";
            }

            obj.html(html);
        }

        function getEAIInspectionInfo(eaiCode) {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxQC.GetEAIInspectionInfo(eaiCode);
            if (ajax.error != null) {
                alert(ajax.error.Message);
                //写入日志
                SaveUserUILog("一般", stationId, resourceId, "", ajax.error.Message);
        ...
        function BindTab() {
            ...
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxQC.GetEAITemplateInfo(IOrderId, TemplateId);
            if (ajax.error != null) {
                alert(ajax.error.Message);
                //写入日志
                SaveUserUILog("一般", stationId, resourceId, "", ajax.error.Message);
        ...
        function getInspectionSN() {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxQC.GetEAISNInfo(IOrderId);
            if (ajax.error != null) {
                alert(ajax.error.Message);
                //写入日志
                SaveUserUILog("一般", stationId, resourceId, "", ajax.error.Message);
        ...
        function getChooseValue1(list) {
            switch (globalFlag) {
                ...
                case 44:   //选择工单
                    ...
                    entity.ItemID = -1;
                    entity.SystemType = 7;//模板类型 1：IQC;2:IPQC;3:PQC;4:FQC;5:OQC;6:FAI;7:EAI
        ...
        function ClearAll() {
            ...
            $("#txtTemplate").val("");
            $("#btnCollectSN,#btnSaveEAICode").show();
            $("#sltEAICode").html("<option value=''>--请选择--</option>");
            $(tab).find(".ListTableOddRow").empty().remove();
        }
