<%@ Page Language="C#" MasterPageFile="~/Masters/ProductionCollection.Master" AutoEventWireup="true" CodeBehind="PrototypeTest.aspx.cs" Inherits="SKT.LeanMES.Web.Client.PrototypeTest" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <style type="text/css">
        #divPanel { width: 100%; height: 180px; background-color: White; }

            #divPanel ul li { width: 31%; float: left; border: 1px solid #ccc; margin: 2px 1px 2px 1px; text-align: center; white-space: nowrap; text-overflow: ellipsis; -o-text-overflow: ellipsis; overflow: hidden; }

                #divPanel ul li:hover { width: 31%; float: left; border: 1px solid green; margin: 2px 1px 2px 1px; }

        .ListTable .TextBox { height: 22px; line-height: 22px; }
    </style>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var scanSN = "";
        var isSN = 0;
        var identityIndex = 0;//自增数（扫描同一条码或拼板不良时自增，过站后，重置；联晶不良代码允许重复扫描，但是不良位置可以不一样）
    
        $(document).ready(function () {
            //投入过站必须选择工单后才可以进行扫描动作
            $("#txtSN").prop("disabled", true);
            ...
            setTimeout(
                function () {
                    loadClientButton('PrototypeTest');
                },
                10
            );
            ...
            $(".nc-position").live("blur", function (event) {
                //if (event.keyCode == "13") {
        ...
        */
        function getChangeOrderValue(list) {
            ...
            if (chooseFlag == 1) {
                ...
                //验证首件逻辑.
                //var ajax1 = SKT.LeanMES.Web.AjaxServices.AjaxClientController.CheckOrderFAI($("#hdnCurrProOrderId").val());
                //if (ajax1.error != null) {
                //    //写入日志
                //    SaveUserUILog("一般", stationId, resourceId, "", ajax1.error.Message);
                //    showAreaMessge(ajax1.error.Message, "messageRed");
                //    return false;
                //}//
                $("#txtSN").removeAttr("disabled");
        ...
        function collectionPosition(object) {
            ...
            var index = obj.closest("tr").attr("Index");//序号
            ...
            if (isPanel == 1) {
                ...
                for (var i = 0; i < panelSNArr.length; i++) {
                    ...
                    if (panelSNArr[i].SN == sn && ncObj != null && ncObj.length > 0) {
                        ...
                        for (var j = 0; j < ncObj.length; j++) {
                            if (ncObj[j].NCCode == ncCode && ncObj[j].Index == index) {
                ...
                for (var i = 0; i < ncCodeArr.length; i++) {
                    if (ncCodeArr[i].NCCode == ncCode && ncCodeArr[i].Index == index) {
        ...
        */
        function afterScan() {
            if ($.trim($("#txtSN").val()) != "") {
                ...
                //获取不良代码
                var arrNcCode = [];
                $("#nccodeList td.tr-nc-code").each(function () {
                    var ncCode = $.trim($(this).text());
                    if (arrNcCode.indexOf(ncCode) == -1) {
                        arrNcCode.push(ncCode);
                    }
                });
                var entity =
                {
                    SN: scanSN,
                    ProdOrderId: parseInt(proOrderId),
                    StationId: parseInt(stationId),
                    ResourceId: parseInt(resourceId),
                    NcCodes: arrNcCode.join(","),
                    ModibyBy: userName,
                }
                var ajax = SKT.AjaxCommon.DBService.ExecuteSpc("uspPrototypeSNCollection", JSON.stringify(entity));
                if (ajax.error != null) {
                ...
                if (list.length > 0 && list[0].SN) {
                    //样机采集成功
                    if (list[0].IsPrototypeSN == 1 && list[0].IsSN == 0) {
                        updateCollectionList(scanSN, 'OK');
                        showAreaMessge(scanSN + ':样品采集通过', "messageGreen");
                        isPanel = -1;
                        panelSNArr = [];
                        scanType = 0;
                        panelId = 0;
                ...
                if (isPanel <= 0 && scanType == 0) {//如果当前SN不是拼板条码 
                    ...
                    if (scanType == 0 && ncCodeArr.length > 0) {//当前没有扫描过拼版扫描且有扫描不良代码
                        ...
                        var panelSNList = [];
                        var ajax = SKT.LeanMES.Web.AjaxServices.AjaxClientController.GetPanelListInfoById(panelId, scanSN);
                        if (ajax.error != null) {
                            updateCollectionList(scanSN, 'NG');
                            showAreaMessge(scanSN + ':' + ajax.error.Message, "messageRed");
                            $("#txtSN").val("").focus();
        ...
        **/
        function loadingNCTable(list, index) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                if (i % 2 == 0) {
                    row.className = 'ListTableOddRow nc-row';
                }
                else {
                    row.className = 'ListTableEvenRow nc-row';
                }
                row.setAttribute("Index", list[i].Index);

                cell = row.insertCell(0);
                cell.align = "center";
                cell.className = "tr-nc-code";
