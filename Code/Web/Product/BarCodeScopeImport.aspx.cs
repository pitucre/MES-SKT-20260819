...
{
    public partial class BarCodeScopeImport : BasePage
    {
        ...
        protected void linkUploadFile_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (fuPickList.HasFile)
                {
                    ...
                    Column += "F" + txtColumn1.Text+ " as SerialNumber";
        ...
        /// </summary>
        /// <param name="dt"></param>
        public void SaveBarCodeImport(DataTable dt)
        {
            List<BarCodeScopeInfo> list = null;
            try
            {
                BarCodeScope bll = new BarCodeScope();
                string OrderNo = txtOrderNo.Text;//工单
                string CustomerOrder = this.CustomerOrder.Text;//订单
                string NumberType = ddlNumberType.SelectedValue;//号码类型
                string NumberClass = ddlClass.SelectedValue;//号码分类
                string IsMain = ddlIsMain.SelectedValue;// 是否主条码
                list = bll.SaveBarCodeImport(OrderNo, CustomerOrder, NumberType, NumberClass, IsMain, AccountController.GetCurrentUser().UserName, dt);
                ...
                Page.ClientScript.RegisterStartupScript(this.GetType(), "tempclick", "<script> ShowErrorBarCode(" + jsonStr + "); </script>");
