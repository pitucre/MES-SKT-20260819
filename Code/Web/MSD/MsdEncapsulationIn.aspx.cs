...
{
    public partial class MsdEncapsulationIn : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(MsdBakeEdit));
            if (!this.IsPostBack)
            {
        ...
        public  MsdItemBakeInfo GetInfoItemBake(int temperature,string serialNumber)
        {
            try
            {
                return new SKT.LeanMES.MSD.BLL.MsdBake().GetInfoItemBake(temperature, serialNumber);
            }
            catch (Exception ex)
            {
        ...
        [AjaxMethod]
        public void BatchAddBake(int containerId, string bakeXml)
        {
            ...
            try
            {
                ...
                SKT.Common.DAL.Marshal.SQLHelper.ExecuteNonQueryStoredProcedure(SKT.Common.DAL.Marshal.SQLHelper.MESConnString, "Msd_Proc_BakeIn", parms);
            }
            catch (Exception ex)
            {
                WebHelper.HandleException(ex);
