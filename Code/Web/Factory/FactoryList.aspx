<%@ Page Language="C#" MasterPageFile="~/Masters/ListMaster.master" AutoEventWireup="True"
    CodeBehind="FactoryList.aspx.cs" Inherits="SKT.LeanMES.Web.Factory.FactoryList"
    Title="Factory List Page" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="GridviewContent" runat="Server">
    <asp:GridView ID="GridView1" runat="server" DataSourceID="ObjectDataSource1" OnRowDataBound="GridView1_RowDataBound">
        <Columns>
            ...
            <asp:BoundField DataField="FactoryCode" HeaderText="<%$ Resources:lang, FactoryCode %>" />
            ...
            <asp:BoundField DataField="ModifyDateTime" HeaderText="<%$ Resources:lang, ModifyDateTime %>"  DataFormatString="{0:yyyy-MM-dd HH:mm:ss}"/>
             <asp:BoundField DataField="IsDefaultFactory" HeaderText="<%$ Resources:lang, IsDefaultFactory %>"  />
    ...
    <script type="text/javascript">
        ...
        function Add() {
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/Factory/FactoryEdit.aspx?name=FactoryAdd&TypeId=1&ID=-1";
            dialog({ title: "<%=Resources.Pages.FactoryAdd %>", src: openWinUrl, width: 450, height: 320 });
        }

        function Edit() {
            var idStr = getOneRecordId();
            if (idStr == "") return false;
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/Factory/FactoryEdit.aspx?name=FactoryEdit&ID=" + idStr;
            dialog({ title: "<%=Resources.Pages.FactoryEdit %>", src: openWinUrl, width: 450, height: 320 });
        }
        ...
        function View() {
            var idStr = getOneRecordId();
            if (idStr == "") return false;
            openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/Factory/FactoryView.aspx?name=FactoryView&ID=" + idStr;
            dialog({ title: "<%=Resources.Pages.FactoryView %>", src: openWinUrl, width: 450, height: 320 });
        }
        function Refresh() {
            document.forms[0].submit();
        }
