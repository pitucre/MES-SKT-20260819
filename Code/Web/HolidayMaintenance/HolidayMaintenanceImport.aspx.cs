...
{
    public partial class HolidayMaintenanceImport : System.Web.UI.Page
    {
        ...
        protected void ButSave_Click1(object sender, EventArgs e)
        {
            ...
            try
            {
                if (fuLoadingList.HasFile)
                {
                    ...
                    if (dtds != null && dtds.Rows.Count > 0)
                    {
                        ...
                        string StrSql = "SELECT Date,Multiple,IsHoliday from  [" + strCom + "]  ";
                        ...
                        DataTable dts = new DataTable("HolidayMaintenance");
                        DataColumn dc = null;
                        dc = dts.Columns.Add("Date", Type.GetType("System.String"));
                        dc = dts.Columns.Add("Multiple", Type.GetType("System.String"));
                        dc = dts.Columns.Add("IsHoliday", Type.GetType("System.String"));
                        DataRow dr;
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (dt.Rows[i][0].ToString() != "")
                            {
                                ...
                                dr["Multiple"] = dt.Rows[i][1].ToString();
                                dr["IsHoliday"] = dt.Rows[i][2].ToString();
                                dts.Rows.Add(dr);
            ...
            catch (Exception ex)
            {
                ...
                /* Wesley:2023-10-24
                 * BUG:1329:节假日模版导入后整个页面点击不了
                 * 原因：以下逻辑要处理异常，结果导致整个系统在不断的报错，最终导致系统死循环了
                 * 优化：只把错误显示给用户即可，现不要处理异常
                 */
