<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ViewMaster.master" AutoEventWireup="true"
    CodeBehind="PlanSchedulView.aspx.cs" Inherits="SKT.LeanMES.Web.Plan.PlanSchedulView" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="viewcontent" runat="server">
    ...
    <table id="tblExpand" cellspacing="0" cellpadding="4" style="border-width: 0px; width: 100%;
        ...
        <tr class="ListTableHeader">
            ...
            <th scope="col" id="thAddDetail"  style=" 
                 width: 8%;">
                 状态
            </th>
        </tr>
    ...
    <script type="text/javascript">
        ...
        function initFTable() {
            // 如果工单未排产，可排产数量就显示计划生产数量 禅道ID: 1222 by: zaiqing.li
            // linePlanType为空表示没有排产
            var fQty = $("#<%= this.lbFQty.ClientID %>").text()
            if (!linePlanType) {
                $("#<%= this.lbFQtyPlan.ClientID %>").text(parseInt(fQty));
            }
        ...
        function addDetail(entity, tableName, lineId, lineName, planQty) {
            ...
            cell.innerHTML = entity.LineName + " <input type=\"hidden\" class=\"hdLineId\" value=\"" + entity.LineId + "\" />"
            + "<input type=\"text\" name=\"txtLineName\"  IsRequired='1' class=\"TextBox\" value=\"" + entity.LineName + "\" disabled=\"disabled\" style=\" width:100px;display:none;\" >"
            ...
            cell.align = "center";
            cell.className = "Field";
            cell.innerHTML = entity.LineMachineRelation;

            var readonly = (entity.State == 2 ? "readonly=\"readonly\"" : "");

            cell = row.insertCell(2);
            cell.align = "center";
            cell.className = "Field";
            cell.innerHTML = entity.FQty+ "<input type=\"text\"  IsNumber='1' onkeyup=\"getIntVal(this)\" IsRequired='1' MinValue='1' " + readonly + " style=\"width: 70px;display:none;\" class=\"fQtyPlan\" value=\"" + entity.FQty
            + "\" onblur=\"onTextBlur(this)\"  />"

            cell = row.insertCell(3);
            cell.align = "center";
            cell.className = "Field";
            cell.innerHTML =  entity.FPlanCommitDate;

            //面别
            var tableStr = getPlanTable(entity.TableName);
            ...
            intiDatepicker($(tdObj[3]).find(".startDate"));
        ...
        function getChooseValueLine(list) {
            ...
            var lineQty = 0;
            if (lineId != -1) {
                for (var i = 0; i < lineIdObj.length; i++) {
                    if (parseInt(lineId) == parseInt($(lineIdObj[i]).val())) {

                        lineQty++;
                        if (lineQty >= 2) {
                            alert('此产线已完成T/B面别的排产！');
                            deleteItem(rowIndex);
                            return false;
                        }
                    }
        ...
        **/
        function getPlanTable(tableName) {
            ...
            if (list != null && list != undefined && list.length > 0) {
                selectStr = "<select name='sltTableName' onchange='setStandardCapacity(this)'  IsRequired='1' style='display:none;' ><option value=''>--请选择--</option>";
        ...
        function setStandardCapacity(obj) {
            ...
            if (!(parseInt(standardCapacity) > 0) && tableName != "") {
            ...
            //检验不允许存在同线别两个A或B面的情况。
            var tableNameQty = 0;
            $("input[type=hidden][class=hdLineId][value=" + lineId + "]").parent().parent().each(
                function () {
                    if ($(this).find("select[name=sltTableName]").val() == tableName) {
                        tableNameQty++;
                        if (tableNameQty > 1) {
                            alert("线别" + lineName + "已存在面别为" + tableName + "的排产信息！");
                            sltObj.val("");
                            flag = false;
                            return false;
                        }
