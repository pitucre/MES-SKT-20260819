<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/EditMaster.master" AutoEventWireup="true"
    CodeBehind="PlanInsertOrder.aspx.cs" Inherits="SKT.LeanMES.Web.Plan.PlanInsertOrder" %>

<asp:Content ID="Content1" ContentPlaceHolderID="EditContent" runat="server">
    <table width="100%" class="EditeContentTable">
        <tr>
            <td class="Label3">
                <%=Resources.lang.OrderNum%><em>*</em>
            </td>
            <td class="Field3">
                <asp:TextBox ID="txtOrderNo" runat="server" CssClass="TextBox" Enabled="false"></asp:TextBox><input
                    type="button" id="btnSelectCustomer" class="ButtonBox" value="..." title="<%=Resources.lang.OrderNum %>"
                    onclick="selectOrder();" />
                <asp:HiddenField ID="hdnOrderId" runat="server" Value="-1" />
            </td>
            <td class="Label3">排产类型<em>*</em>
            </td>
            <td class="Field3">
                <asp:DropDownList runat="server" ID="ddlPlanType" ClientIDMode="Static">
                    <asp:ListItem Value="-1" Selected="True">=请选择=</asp:ListItem>
                    <asp:ListItem Value="1">SMT排产</asp:ListItem>
                    <asp:ListItem Value="2">其他排产</asp:ListItem>
                </asp:DropDownList>
            </td>
    ...
    <script type="text/javascript">
        ...
        var planType = 1; //排产类型：1、SMT工单排程 2、其他工单排程     
        //产量计划
        var fQty = 0;
        //求排期数量lb对象
        ...
        function initFTable(orderId) {

           
            $("#tblExpand tr:not(:first)").remove();
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxPlan.GetLinePlanByFInterID(orderId);
            IsInti = true;
                ...
                if (entityAry.length > 0) {
                    for (var i = 0; i < entityAry.length; i++) {
                        if (i == 0) {
                            ...
                            $("#<%=this.ddlPlanType.ClientID %>").val(planType);
                        }
        ...
        function addDetail(entity, tableName, lineId, lineName, planQty) {
            ...
            planType = $("#<%=this.ddlPlanType.ClientID %>").val();
            if (planType == -1) {
                alert("请选择排产类型");
                return false;
            }
        ...
        function Save() {
            if (checkBalancePlanQty()) {
                ...
                var productLoadArr = GetArrValue($("span[name=lblProductLoad]"),1);
              
                var entity = {};
                entity.LinePlanIdArr = LinePlanIdArr;
                entity.FInterID = $("#<%=this.hdnOrderId.ClientID %>").val();
                entity.FBILLNO = $("#<%=this.txtOrderNo.ClientID %>").val();
                ...
                entity.ProductLoadArr = productLoadArr;
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxPlan.CollectOrderPlanInsert(entity);
        ...
        function selectItems(obj) {
            ...
            if ($(rowObj).find(".startDate").val() == "") {
                $(rowObj).find(".startDate").val('').focus();
                alert("请选择开始时间");
                return false;
            };
            dialog({
        ...
        function onTextBlur(obj) {
            ...
            if (hdWorkHour > 0 && lblStandardCapacity != '') {
                ...
                $(obj).parent().parent().find("span[name=lblProductLoad]").text(((parseFloat($(obj).val()) - parseFloat(hdPlanNumber)) / cap / hdWorkHour * 100 + parseFloat(hdProductLoad)).toFixed(2));
            }
        ...
        **/
        ...
        function setStandardCapacity(obj) {
            ...
            if ((!(parseInt(standardCapacity) > 0) && tableName != "" && planType == 1) || standardCapacity == "未维护") {
    ...
    <script type="text/javascript">
        ...
        function selectOrder() {
            ...
            dialog({ title: "<%=Resources.Common.ChooseWindow %>", src: "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>/Framework/ChoosePage.aspx?PageId=607&Multiple=false&rnd=" + Math.random(), width: 600, height: 350 });
        }

        function getChooseValue(list) {
           
            getOrderInfo(list[0][0]);
        }

        function getOrderInfo(orderId) {
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxShopOrder.GetInfo(orderId);
            if (ajax.error == null) {
                var entity = ajax.value;
                if (entity != null) {

                    if (entity.RouterName == "") {
                        alert("未找到当前工单的路由信息，请先维护路由信息！");
                        return false;
                    }
                    $("#<%=this.txtOrderNo.ClientID %>").val(entity.OrderNO);
                    $("#<%=this.hdnOrderId.ClientID %>").val(orderId);
                    $("#<%=this.lblItemCode.ClientID %>").text(entity.ItemCode);
                    $("#<%=this.lbFQty.ClientID %>").text(entity.Qty_to_Build);
                    $("#<%=this.lbFName.ClientID %>").text(entity.ItemName);
                    $("#<%=this.lbFPlanCommitDate.ClientID %>").text(entity.Planned_Start_Time);
                    $("#<%=this.lbFPlanFinishDate.ClientID %>").text(entity.Planned_Completed_Date);
               <%--     $("#<%=this.lblPlaneQty.ClientID %>").text(entity.PanelQty);--%>
                    $("#<%=this.lblRouterName.ClientID %>").text(entity.RouterName);
                    $("#<%=this.hdnItemId.ClientID %>").val(entity.ItemId);
                    $("#<%=this.lbFQtyPlan.ClientID%>").text(entity.Qty_to_Build);

                    
                    fQty = entity.Qty_to_Build;
                    initFTable(orderId);
