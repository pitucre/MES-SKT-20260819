<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/ListMaster.master" AutoEventWireup="true"
    CodeBehind="ProductionControlListStandard.aspx.cs" Inherits="SKT.LeanMES.Web.Plan.ProductionControlListStandard" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="GridviewContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function View() {
            ...
            // 5 改为 ResName
            var itemcode = getOneRecordCellTextByFiled("ResName");
            if (resName == "默认资源") {
    ...
    **/
    function AddPostionAndReplaceItem() {
        ...
        openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/Plan/StockListEditStandard.aspx?name=Plan_StockList_Standard&ID=" + id + "&PlanOrderNo=" + escape(planOrderNo);
    ...
    **/
    function ChangeLine() {
        ...
        openWinUrl = "<%=SKT.LeanMES.Web.WebHelper.WebRoot %>" + "/Plan/StockListChangeOrderStandard.aspx?name=Plan_ChangeLine_Standard&ID=" + id + "&PlanOrderNo=" + escape(planOrderNo);
    ...
    **/
    ...
    //上料清单替换
    function ReplaceLoadList() {
        var id = getOneRecordId();
        if (id == "") return false;
        if (confirm("是否确定要进行上料清单替换操作！")) {
            //xiang.yan 2024-4-28  列取值由索引改为列明
            // 1 改为 FBILLNO
            var planOrderNo = getOneRecordCellTextByFiled("FBILLNO");
            var ajax = SKT.LeanMES.Web.AjaxServices.AjaxPlan.IsExistStockList(planOrderNo);
            if (ajax.error != null) {
                alert(ajax.error.Message);
                return false;
            } else {
                if (ajax.value != "0") {
                    if (confirm("该排程单[" + planOrderNo + "]已有上料记录，如果仍需替换系统会自动清除上料记录？")) {
                        ReplaceLoadList2(planOrderNo);
                    }
                } else {
                    ReplaceLoadList2(planOrderNo);
                }
    ...
    function ReplaceLoadList2(planOrderNo) {
        var ajax = SKT.LeanMES.Web.AjaxServices.AjaxPlan.ReplaceLoadList(planOrderNo);
        if (ajax.error != null) {
            alert(ajax.error.Message);
            return false;
        }
