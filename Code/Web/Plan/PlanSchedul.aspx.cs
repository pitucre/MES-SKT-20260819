...
{
    public partial class PlanSchedul : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            searchSettings.ExtensionCondition = " IsHand=1 ";
            if (hfStrOrderStatus.Value != "")
            {
                var status = hfStrOrderStatus.Value.ToString();
                var where = "";
                if (status != "-10")
                {
                    where += "and TableName != ''";
                }
                searchSettings.ExtensionCondition = $" [StatusId] IN ({status}) {where}";
            }
