<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CPReturnStockEx.aspx.cs" Inherits="SKT.LeanMES.Web.MobileApp.CPReturnStockEx" %>
    ...
    <form id="form1" runat="server">
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $("#txtSN").on("keydown", function (e) {
                    ...
                    if (curKey == 13) {
                        ...
                        var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.SaleReturnScanSNEx(entity);
                        if (ajax.error != null) {
                            showMsg(ajax.error.Message, 0);
                            $("#txtSN").val("").focus();
                            return false;
                ...
                $("#saleReturnSN tbody").on("click", "a.delete-sn", function () {
                    ...
                    var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.SaleReturnDeleteSNEx(entity);
                    if (ajax.error != null) {
                        showMsg(ajax.error.Message, 0);
                        return false;
                    }
            ...
            function searchSaleReturnNo(saleReturnNo) {
                $("#listviews").html("");
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.GetSaleReturnListEx({ DocNo: saleReturnNo });
                if (ajax.error != null) {
                    showMsg(ajax.error.Message, 0);
                    $("#txtSaleReturnNo").val("").focus();
                    return false;
                ...
                for (var i = 0; i < list.length; i++) {
                    ulhtml += "<li class='ui-btn ui-btn-icon-right ui-icon-carat-r'><a onclick='getSaleReturnNoDetailInfo(" + (JSON.stringify(list[i])) + ",1)'>" + list[i].DocNo + "</a></li>";
                }
            ...
            function getSaleReturnNoDetailInfo(entity, type) {
                ...
                if (type == 0) {
                    ...
                    //根据送货单信息加载供应商信息
                    var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.GetSaleReturnExInfo({ DocNo: saleReturnNo, CreateBy:userName });
                    if (ajax.error != null) {
                        showMsg(ajax.error.Message, 0);
                        $("#txtSaleReturnNo").val("").focus();
                        return false;
                    ...
                    //选择单据后，获取换货明细信息
                    $("#txtSaleReturnNo").val(entity.DocNo);
                ...
                getSaleReturnDtlList(entity.DocNo);
                getSaleReturnScanList(entity.DocNo);
                $("#CBarCode").focus();
            }

            //获取退货物料信息
            function getSaleReturnDtlList(saleReturnNo) {
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.GetSaleReturnDetailEx({ DocNo: saleReturnNo, CreateBy:userName });
                if (ajax.error != null) {
                    showMsg(ajax.error.Message, 0);
                    $("#txtSaleReturnNo").val("").focus();
                    return false;
            ...
            function getSaleReturnScanList(saleReturnNo) {
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.GetSaleReturnScanExList({ DocNo: saleReturnNo });
                if (ajax.error != null) {
                    showMsg(ajax.error.Message, 0);
                    return false;
                }
                ...
                if (list != null) {
                    for (var i = 0; i < list.length; i++) {
                        hl += "<tr>" +
                            ...
                            "<td>" + list[i].Qty + "</td>" +
                            "<td><a href=\"javascript:void(0)\" class=\"delete-sn\" SaleReturnNo=\"" + list[i].DocNo + "\" BarCode=\"" + list[i].BarCode + "\">删除</a></td>" +
                            "</tr>";
                    }
                }
                return hl;
            ...
            function Save() {
                ...
                //var flag = true;
                //$("#saleReturnDetail tbody tr").each((index,item) => {
                //    if ($(item).find(".saleReturnQty").text() != $(item).find(".currentReturnQty").text()) {
                //        flag = false;
                //    }
                //});
                //if (!flag) {
                //    $("#msg").html('入库数量与申请数量不一致，请继续扫描').css('color', '#ff0000');
                //    return false;
                //}

                //保存
                var ajax = SKT.LeanMES.Web.AjaxServices.AjaxSaleReturn.SaleReturnExSave({ DocNo: saleReturnNo, BarCode: cBarCode });
                if (ajax.error != null) {
                    showMsg(ajax.error.Message, 0);
                    $("#txtSN").val("").focus();
                    return false;
