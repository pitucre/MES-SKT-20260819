<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ContainerPrint.aspx.cs" Inherits="SKT.MES.Web.BasalData.ContainerPrint" MasterPageFile="~/Masters/ViewMaster.master" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="viewcontent" runat="server">
    ...
    <table width="100%" class="EditeContentTable">
        ...
        <tr>
            ...
            <td class="Field1">
                <input type="text" id="txtQuantity" class="TextBox" style="height: 25px; width: 250px;
                    text-transform: uppercase; font-size: 16px; font-weight: bold;" onkeyup="this.value=this.value.replace(/\D/g,'')"
                    onafterpaste="this.value=this.value.replace(/\D/g,'')" />
            </td>
        </tr>
    ...
    <script type="text/javascript">
       ...
       <% } %>
        ...
        function Print() {
            ...
            var packQuantity = $("#txtPackMaxQuantity").val();
            if(parseFloat(packQuantity) > parseFloat(maxQuantity))
            {
               alert("包装数量不能大于最大包装数量!");
               return  false ;
            }

            var  nextId = -1;
            /***获取规则ID****/
            var ajaxValue = SKT.LeanMES.Web.AjaxServices.AjaxContainer.GetNextIDByTypeValue(typeValue);
            ...
            if(ContainerID !=-1)
            {
               var ajaxPOD = SKT.LeanMES.Web.AjaxServices.AjaxContainer.PrintMoreCaronSNStr(nextId ,Quantity,  ContainerID , -1 , -1 , userId);
                    if (ajaxPOD.error == null) {
                        conTainerNumber = ajaxPOD.value;
                    }
                    else {
                        alert(ajaxPOD.error.Message);
                        return false;
                    }
            }

            /*获取工单等信息*/
            /* findInfoByOrderNo(orderNo);*/
            /*End*/
            /**********开始打印*************************/
            if (!window.confirm("是否确定打印标签？")) {
                return false;
            }
        ...
        /******批量打印条码*******/
          ...
          function printCartonSN(conNumberStr) {
            ...
            {
               if(conNumberArr.length)
               {
                 var s = "... ...";
                 iCount = (iCount > 6) ? 0 : (iCount + 1);
                 $("#lblPt").html("正在排队打印，还有" + conNumberArr.length.toString() + "个条码等待打印" + s.substring(0, iCount));
                 maxQuantity = maxQuantity +'';
                 date ="";
                 var __cartonSN = conNumberArr.shift();
                 var date ="";   //日期
                 var ajaxLabel = SKT.LeanMES.Web.AjaxServices.AjaxContainer.GetCartonLabelData(ContainerID, orderNo, maxQuantity,__cartonSN,modelSpec, txtLeft);
                 ...
                 if (ajaxLabel.value != "") {                    
                    ...
                    setTimeout(printCartonNumber, 500);
                  }
                  else {
                    alert("标签内容为空！");
                    return false;
