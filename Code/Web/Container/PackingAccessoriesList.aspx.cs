...
{
    public partial class PackingAccessoriesList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.Master.RecordIDField = "PackingAccessoriesConfigId";
            this.Master.DefaultSortExpression = "PackingAccessoriesConfigId DESC"; //也可不赋值

            SKT.Common.Model.SearchSettings searchSettings = new SKT.Common.Model.SearchSettings();
            searchSettings.AddCondition("ItemCode", txtItemCode.Text.Trim().Replace("'","''"));
            searchSettings.AddCondition("AccessoriesName", txtAccessoriesName.Text.Trim().Replace("'", "''"));
            searchSettings.AddCondition("Station", txtStation.Text.Trim().Replace("'", "''"));
            ...
            if (IsPostBack)
            {
                if (Request.Form["hdnOperate"].ToLower() == "delete")
                {
                    try
                    {
                        SKT.LeanMES.Container.BLL.PackingAccessories bll = new SKT.LeanMES.Container.BLL.PackingAccessories();
