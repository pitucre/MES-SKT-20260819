...
{
    public class EquipmentInspectionTemplateMember
    {
        ...
        public Int32 Edit(EquipmentInspectionTemplateMemberInfo entity)
        {
        ...
        public EquipmentInspectionTemplateMemberInfo GetInfo(Int32 inspectionTemplateMemberId)
        {
            EquipmentInspectionTemplateMemberInfo entity = null;
            ...
            using (SqlDataReader rdr = SQLHelper.ExecuteReaderStoredProcedure(SQLHelper.MESConnString, "Quality_InspectionTemplateMember_GetInfo", parms))
            {
                if (rdr.Read())
                {
                    entity = new EquipmentInspectionTemplateMemberInfo(rdr.GetInt32(0), rdr.GetInt32(1), rdr.GetInt32(2), rdr.GetInt32(3), rdr.GetString(4),
        ...
        /// <returns>InspectionTemplateMember 实体对象。</returns>
        public EquipmentInspectionTemplateMemberInfo GetInfo(String fieldValue)
        {
            EquipmentInspectionTemplateMemberInfo entity = null;
        ...
        public List<EquipmentInspectionTemplateMemberInfo> GetAll(Int32 startRow, Int32 maxRows, String sortExpression, SearchSettings searchSettings)
        {
            List<EquipmentInspectionTemplateMemberInfo> list = new List<EquipmentInspectionTemplateMemberInfo>();
            //表名或者视图
            string strTb = "[dbo].[Equipment_InspectionTemplateMember] a	 INNER JOIN [dbo].[Equipment_InspectionTemplate] b ON b.InspectionTemplateId = a.InspectionTemplateId	INNER JOIN dbo.Equipment_InspectionItem f ON f.InspectionItemId = a.InspectionItemId LEFT JOIN dbo.Quality_AQLRule g ON g.AQLRuleId = a.aqlruleId";
            //主键
            ...
            list = ComMethod.GetComList<EquipmentInspectionTemplateMemberInfo>(ref recordCount, startRow, maxRows, strTb, strKey, strColumns, sortExpression, searchSettings);
        ...
        /// <param name="inspectionTemplateMemberCount">inspectionTemplateMember 总数。</param>
        /// <returns>InspectionTemplateMember 列表。</returns>
        public List<EquipmentInspectionTemplateMemberInfo> GetAllJW(Int32 startRow, Int32 maxRows, String sortExpression, SearchSettings searchSettings)
        {
            List<EquipmentInspectionTemplateMemberInfo> list = new List<EquipmentInspectionTemplateMemberInfo>();
            //表名或者视图
            string strTb = "[dbo].[Equipment_InspectionTemplateMember] a	 INNER JOIN [dbo].[Equipment_InspectionTemplate] b ON b.InspectionTemplateId = a.InspectionTemplateId	INNER JOIN dbo.Equipment_InspectionItem f ON f.InspectionItemId = a.InspectionItemId LEFT JOIN dbo.Quality_AQLRule g ON g.AQLRuleId = a.aqlruleId  LEFT JOIN  Basal_Station as Sta  on  Sta.StationId=a.OpenId";
            //主键
            string strKey = "InspectionTemplateMemberId";
            //查询栏位字串
            string strColumns = @"[InspectionTemplateMemberId],
	a.[InspectionTemplateId],
	a.[InspectionItemId],
	a.[Sorting],
	a.[MaxValue],
	[MinValue],
	...
	a.[ModifyDateTime],
	a.[Remark],
	f.InspectionItemName,
	InspectionAccording,
	IsPercentage,
	a.aqlruleid,
	g.RuleName,
	a.InspectJuge,
	f.InspectionMethodId,
	a.InspectionMethodValue
	,
	a.UnitName,
	IsNull( a.CheckFashion, '' ) AS CheckFashion,
	IsNull( f.TestMethod, '' ) AS TestMethod,
	[OpenId],
	ISNULL(
		(
		SELECT top 1
			InspectionImageName
		FROM
			dbo.Equipment_InspectionTemplateMemberImg myi 
		WHERE
			myi.InspectionTemplateId=  a.InspectionTemplateId  
			AND myi.InspectionItemId= a.[InspectionItemId] ORDER BY CreateDateTime desc
		),
		'' 
	) AS OpenName,
	isnull( a.IsUpLoadImg, 0 ) IsUpLoadImg,
	IsNull( a.NoteText, '' ) AS NoteText ";
            list = ComMethod.GetComList<EquipmentInspectionTemplateMemberInfo>(ref recordCount, startRow, maxRows, strTb, strKey, strColumns, sortExpression, searchSettings);
        ...
        public List<EquipmentInspectionTemplateMemberInfo> GetInspectionMemberInfoById(Int32 IOrderId)
        {
            List<EquipmentInspectionTemplateMemberInfo> list = new List<EquipmentInspectionTemplateMemberInfo>();
            EquipmentInspectionTemplateMemberInfo entity = null;
            SqlParameter[] parms = new SqlParameter[] {
            ...
            using (SqlDataReader rdr = SQLHelper.ExecuteReaderStoredProcedure(SQLHelper.MESConnString, "uspGetInspectionOrderById", parms))
            {
                while (rdr.Read())
                {
                    entity = new EquipmentInspectionTemplateMemberInfo();
        ...
        public List<EquipmentInspectionTemplateMemberInfo> GetAllJoinItem(Int32 startRow, Int32 maxRows, String sortExpression, SearchSettings searchSettings)
        {
            List<EquipmentInspectionTemplateMemberInfo> list = new List<EquipmentInspectionTemplateMemberInfo>();
            EquipmentInspectionTemplateMemberInfo entity = null;

            SqlParameter[] parms = SQLHelper.CreateCommonPagingStoredProcedureParameters(startRow, maxRows, "[dbo].[Equipment_InspectionTemplateMember] a	INNER JOIN [dbo].[Equipment_InspectionTemplate] b ON b.InspectionTemplateId = a.InspectionTemplateId	INNER JOIN [dbo].[Equipment_InspectionTemplateItem] c ON c.InspectionTemplateId = b.InspectionTemplateId	INNER JOIN dbo.Basal_Item d ON c.ItemID = d.ItemID 	INNER JOIN [dbo].[Quality_InspectionOrder] e ON e.ItemCode = d.ItemCode AND e.InspectionTypeId = b.InspectionTypeId	INNER JOIN dbo.Equipment_InspectionItem f ON f.InspectionItemId = a.InspectionItemId"
