...
*
* The Original Code is 'PDF417, a library to generate the bidimensional barcode PDF417'.
*
...
* http://sourceforge.net/projects/PDF417
...
public class PDF417
{
    ...
    internal protected sbyte[] Filter(string sValue)
    {
        ...
        //byte[] sArrayTemp = System.Text.Encoding.UTF8.GetBytes(sValue);


        //sbyte[] sArray = new sbyte[sArrayTemp.Length];
        ////Buffer.BlockCopy(data, 0, sdata, 0, data.Length);
        //Buffer.BlockCopy(sArrayTemp, 0, sArray, 0, sArrayTemp.Length);
    ...
    protected internal const int ABSOLUTE_MAX_TEXT_SIZE = 2420;
    protected internal const int MAX_DATA_CODEWORDS = 326;
    ...
    {
        private void InitBlock(PDF417 enclosingInstance)
        {
            this.enclosingInstance = enclosingInstance;
        }
        private PDF417 enclosingInstance;
        public PDF417 Enclosing_Instance
        {
            get
            {
        ...
        public Segment(PDF417 enclosingInstance, char type, int start, int end)
        {
    ...
    {
        public SegmentList(PDF417 enclosingInstance)
        {
            InitBlock(enclosingInstance);
        }
        private void InitBlock(PDF417 enclosingInstance)
    ...
    /// <summary>
    /// 将输入的字符串形成Pdf417二维码编码，并根据传入的参数形成位图
    /// </summary>
    /// <param name="barCodeString">要生成二维码的字符串</param>
    /// <param name="hScale">位图高度</param>
    /// <param name="vScale">位图宽度</param>
    /// <returns>返回位图数据</returns>
    public Bitmap CreatePdf417BitMap(string barCodeString, int hScale, int vScale,Brush fontColor,Brush backColor)
    {
        try
        {
            PDF417 pd = new PDF417();
            //设置要转换的字符串
            pd.setText(barCodeString);
            //设置生成的格式
            pd.Options = PDF417.PDF417_INVERT_BITMAP;
            //生成二维码数据
            pd.paintCode();
            ...
            for (int i = 0; i < out_Renamed.Length; ++i)
            {
                ...
                for (int j = 7; j >= 0; j--)
                {
                    ...
                    if ((value & mask) != 0)
                        g.FillRectangle(backColor, bitcol * hScale, row * vScale, hScale, vScale);
                    else
                        g.FillRectangle(fontColor, bitcol * hScale, row * vScale, hScale, vScale);
                    bitcol++;
            ...
            //bitmap.Save("C:\\1.png", ImageFormat.Png);
    ...
    public static void  Main(System.String[] args)
    {
        if (args.Length < 2)
        {
            ...
            System.Console.Out.WriteLine("Usage: java PDF417 postscript_file_name barcode_text");
            return ;
        }
        try
        {
