...
// update by weixia on 2016.8.19增加方法
// 
// 
//--------------------------------------------------*/
using System;
 ...
 public   class Helper
    {
        ...
        public static T ToEntity<T>(SqlDataReader reader) where T : class
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    PropertyInfo property = entity.GetType().GetProperty(reader.GetName(i), BindingFlags.Instance | BindingFlags.Public | BindingFlags.IgnoreCase);
                    if (property != null)
                    {
                        property.SetValue(entity, Convert.IsDBNull(reader.GetValue(i)) ? null : reader.GetValue(i), null);
                    }
                }
            }
            reader.Close();
