...
namespace BarcodeLibTest
{
    ...
    public partial class TestApp : Form
    {
        ...
        private Dictionary<string, string> _fontFamilys = null;

        public Dictionary<string, string> FontFamilys
        {
            get
            {
                if (_fontFamilys != null)
                    return _fontFamilys;
                _fontFamilys = new Dictionary<string, string>();
                string[] array = Directory.GetFiles(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "font"));
                for (int i = 0; i < array.Length; i++)
                {
                    try
                    {
                        System.Windows.Media.GlyphTypeface glyphTypeface = new System.Windows.Media.GlyphTypeface(new Uri(array[i]));
                        _fontFamilys.Add(glyphTypeface.Win32FamilyNames[new System.Globalization.CultureInfo("en-us")], array[i]);
                    }
                    catch (Exception)
                    {
        ...
        private void btnEncode_Click(object sender, EventArgs e)
        {
            ...
            {
                string fontName = cbxFontFamily.SelectedItem.ToString();
                if (FontFamilys.ContainsKey(fontName))
                {
                    pfc = new PrivateFontCollection();
                    pfc.AddFontFile(FontFamilys[fontName]);
                    b.LabelFont = new Font(pfc.Families[0], 60);
                }
            }
            //barcode alignment
