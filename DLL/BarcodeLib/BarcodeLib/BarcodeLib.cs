...
{
    #region Enums
    public enum TYPE : int { UNSPECIFIED, UPCA, UPCE, UPC_SUPPLEMENTAL_2DIGIT, UPC_SUPPLEMENTAL_5DIGIT, EAN13, EAN8, Interleaved2of5, Standard2of5, Industrial2of5, CODE39, CODE39Extended, Codabar, PostNet, BOOKLAND, ISBN, JAN13, MSI_Mod10, MSI_2Mod10, MSI_Mod11, MSI_Mod11_Mod10, Modified_Plessey, CODE11, USD8, UCC12, UCC13, LOGMARS, CODE128, CODE128A, CODE128B, CODE128C, ITF14, CODE93, TELEPEN, FIM, PHARMACODE, CODE39_Mod43 };
    ...
    public class Barcode : IDisposable
    {
        ...
        /// <summary>
        /// Gets or sets whether a border should be drawn below the image. (Default is false)
        /// </summary>
        ...
        /// <summary>
        /// padding of lable barcode.unit is the count of barWidth.
        /// </summary>
        public int? LabelPaddingCols { get; set; }
        #endregion
                ...
                case TYPE.ITF14:
                    {
                        ...
                        {
                            //限制为最小线宽，避免出现异常
                            int minWidth = (int)(241 / 176.9 * Encoded_Value.Length + 1);
                ...
                case TYPE.UPCA:
                    {
                        ...
                        if (BarWidth.HasValue)
                        {
                            Width = (int)BarWidth * Encoded_Value.Length;
                        }
                        else
                        {
                            //限制为最小线宽，避免出现异常
                            int minWidth = Encoded_Value.Length;
                            Width = Width < minWidth ? minWidth : Width;
                        }
                        ...
                        if (IncludeLabel)
                        {
                            ...
                            {
                                padding = iBarWidth * (LabelPaddingCols ?? 10);
                                // UPCA standardized label
                                string defTxt = RawData;
                                string labTxt = defTxt.Substring(0, 1) + "--" + defTxt.Substring(1, 6) + "--" + defTxt.Substring(7);

                                Font labFont = new Font(this.LabelFont.FontFamily, Labels.getFontsize(Width, Height, labTxt, this.LabelFont.FontFamily), LabelFont.Style);
                                LabelFont = labFont;
                        ...
                        bitmap = new Bitmap(Width + padding * 2, Height);
                ...
                case TYPE.UPCE:
                    {
                        ...
                        if (IncludeLabel)
                        {
                            if ((AlternateLabel == null) || (RawData.StartsWith(AlternateLabel)) && _StandardizeLabel == true)
                            {
                                padding = iBarWidth * (LabelPaddingCols ?? 8);
                                // UPCA standardized label
                                string defTxt = RawData;
                                string labTxt = defTxt.Substring(0, 1) + "--" + defTxt.Substring(1, 6) + "--" + defTxt.Substring(7);
