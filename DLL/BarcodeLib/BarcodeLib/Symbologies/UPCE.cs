    ...
    class UPCE : BarcodeCommon, IBarcode
    {
        ...
        private string Encode_UPCE()
        {
            ...
            if (Raw_Data.Length == 8) {
                UPCECode = Raw_Data.Substring(1, 6);
            }
            else if (Raw_Data.Length == 12) //Convert to UPC-E from UPC-A if necessary
            {
            ...
            Raw_Data = NumberSystem.ToString() + UPCECode + CheckDigit.ToString();
            ...
            {
                int i = Int32.Parse(UPCECode[pos++].ToString());
                if (c == 'a')
                {
                    result += EAN_CodeA[i];
                }//if
